...
#st.table(df)
    ...
    st.write("When the simulation is finished, the user may choose to run prediction and visualization models by changing the 'Selected Page' to analysis")
    st.subheader("Analysis page")
    st.write("The analysis page provides the users with the options of running simple Explanatory Data Analysis (EDA), as well as more sophisticated prediction/clustering models.")
    st.write("At the bottom of the EDA section, the user may choose to download the dataset resulting from the simulation.")
    st.write("If the user choose to clustering, a Kmeans algorithm will be executed with the user specified arguments. Heuristical tools of how many clusters to choose are presented together with a table displaying the proportion of matching recipes (cuisines) and proportion of left swipes in each cluster.")
    st.write("If Decision Tree Analysis is selected a decision tree algorithm will be executed with the user-specified arguments. When the algorithm has converged, model statistics are shown together with a visualization of the tree.")
